...
describe("employment invariants", () => {
  ...
  beforeAll(async () => {
    org = await createTestOrg("employment");
    admin = await createTestUser("Employment Admin");
    await addMembership(org.orgId, admin.userId, ["company_admin"]);
  });
  ...
  it("a new employee gets exactly one OPEN period, and employment_status is 'active'", async () => {
    const employeeId = await createTestEmployee(org.orgId, null, "Fresh", "Hire");
  ...
  it("only one active (open) employment period — a second open period is rejected", async () => {
    const employeeId = await createTestEmployee(org.orgId, null, "One", "Period");
    await expect(
      asUser(admin.userId, (tx) =>
        tx`insert into employee_employment_periods (organization_id, employee_id, start_date) values (${org.orgId}, ${employeeId}, current_date)`,
      ),
  ...
  it("end employment works: closing the open period sets employment_status to 'terminated'", async () => {
    const employeeId = await createTestEmployee(org.orgId, null, "To End", "Employment");
  ...
  it("rehire opens a valid new employment period and restores employment_status to 'active'", async () => {
    const employeeId = await createTestEmployee(org.orgId, null, "Re", "Hired");
    const [period] = await sql`select id from employee_employment_periods where employee_id = ${employeeId} and end_date is null`;
    ...
    const rows = await asUser(admin.userId, (tx) =>
      tx`insert into employee_employment_periods (organization_id, employee_id, start_date) values (${org.orgId}, ${employeeId}, ${rehireStart}) returning id`,
    );
    expect(rows).toHaveLength(1);
  ...
  it("employment periods cannot overlap — a new period starting at or before the prior close date is rejected", async () => {
    const employeeId = await createTestEmployee(org.orgId, null, "Overlap", "Test");
    const [period] = await sql`select id from employee_employment_periods where employee_id = ${employeeId} and end_date is null`;
    ...
    await expect(
      asUser(admin.userId, (tx) =>
        tx`insert into employee_employment_periods (organization_id, employee_id, start_date) values (${org.orgId}, ${employeeId}, ${endDate})`,
      ),
    ...
    await expect(
      asUser(admin.userId, (tx) =>
        tx`insert into employee_employment_periods (organization_id, employee_id, start_date) values (${org.orgId}, ${employeeId}, '2026-01-01')`,
      ),
    ).rejects.toMatchObject(RAISED_EXCEPTION);
  });

  it("invalid date transitions fail: end_date before start_date is rejected by the CHECK constraint", async () => {
    const employeeId = await createTestEmployee(org.orgId, null, "Bad", "Dates");
  ...
  it("invalid date transitions fail: closing a period without end_reason is rejected", async () => {
    const employeeId = await createTestEmployee(org.orgId, null, "No", "Reason");
    const [period] = await sql`select id from employee_employment_periods where employee_id = ${employeeId} and end_date is null`;
    await expect(
  ...
  it("invalid date transitions fail: a closed period can never be modified again", async () => {
    const employeeId = await createTestEmployee(org.orgId, null, "Closed", "Immutable");
    const [period] = await sql`select id from employee_employment_periods where employee_id = ${employeeId} and end_date is null`;
    await asUser(admin.userId, (tx) =>
  ...
  it("employee numbers cannot be changed once assigned", async () => {
    const employeeId = await createTestEmployee(org.orgId, null, "Immutable", "Number");
    await expect(
      asUser(admin.userId, (tx) => tx`update employees set employee_number = 'CHANGED-001' where id = ${employeeId}`),
    ).rejects.toMatchObject(RAISED_EXCEPTION);
  });

  it("the one-open-period invariant holds even for an employee who also holds an elevated organization role", async () => {
    const roleBearer = await createTestUser("Foreman With Employment");
    const foremanRoleId = await roleId("foreman");
    const membershipId = await addMembership(org.orgId, roleBearer.userId, []);
    await sql`insert into membership_roles (organization_id, membership_id, role_id) values (${org.orgId}, ${membershipId}, ${foremanRoleId})`;
    const employeeId = await createTestEmployee(org.orgId, roleBearer.userId, "Foreman", "Employee");
