...
import { replaceToolboxMeetingFile } from "@/modules/toolbox-meetings/actions";
  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col gap-3" noValidate>
      ...
      <div className="flex flex-col gap-1.5">
        <Label htmlFor="replace-file">Corrected PDF</Label>
        <label htmlFor="replace-file" className="flex cursor-pointer flex-col items-center gap-2 rounded-lg border border-dashed p-4 text-center text-sm text-muted-foreground transition-colors hover:bg-muted/50">
          <Upload className="size-5" aria-hidden="true" />
          {fileName ?? "Choose the corrected PDF file"}
        </label>
        <input id="replace-file" name="file" type="file" accept="application/pdf,.pdf" className="sr-only" onChange={(event) => setFileName(event.target.files?.[0]?.name ?? null)} />
