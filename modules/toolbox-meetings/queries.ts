 ...
 * `organization_id` (RLS also enforces this); no PostgREST embeds, same
...
export async function listToolboxMeetings(organizationId: string, filters: ToolboxMeetingListFilters = {}): Promise<ToolboxMeeting[]> {
  const supabase = await createClient();
  let query = supabase.from("toolbox_meetings").select("*").eq("organization_id", organizationId);
...
export async function listRecentToolboxMeetingsForOverview(organizationId: string, filters: ToolboxMeetingListFilters, limit: number): Promise<ToolboxMeeting[]> {
  const results = await listToolboxMeetings(organizationId, filters);
  return results.slice(0, limit);
}

export async function getToolboxMeeting(organizationId: string, meetingId: string): Promise<ToolboxMeetingDetail | null> {
  const supabase = await createClient();
  const { data: meeting, error } = await supabase.from("toolbox_meetings").select("*").eq("organization_id", organizationId).eq("id", meetingId).maybeSingle();
  if (error) throw error;
...
/** Employees eligible to be "the HSE person who held the meeting" for `projectId` — hseq_manager org-wide, or hse_officer assigned to that project. */
export async function listToolboxAuthorizedEmployees(organizationId: string, projectId: string): Promise<{ id: string; first_name: string; last_name: string; employee_number: string; profile_id: string | null }[]> {
  const supabase = await createClient();
  const { data, error } = await supabase.rpc("list_toolbox_authorized_employees", { target_organization_id: organizationId, target_project_id: projectId });
  if (error) throw error;
  return data ?? [];
}

/** Projects the caller can create a toolbox meeting for — org-wide (every non-archived project) if hseq_manager, otherwise every project where they hold a current hse_officer-eligible assignment. Mirrors modules/scaffolds/queries.ts's listScaffoldCreatableProjects exactly, including the "role-gated, not just any project_assignments row" fix. */
export async function listToolboxMeetingCreatableProjects(organizationId: string, userId: string, roleNames: RoleName[]): Promise<Project[]> {
  const supabase = await createClient();
...
export async function getToolboxMeetingOverviewCounts(organizationId: string, projectId?: string): Promise<ToolboxMeetingOverviewCounts> {
  const supabase = await createClient();
  let query = supabase.from("toolbox_meetings").select("*").eq("organization_id", organizationId);
