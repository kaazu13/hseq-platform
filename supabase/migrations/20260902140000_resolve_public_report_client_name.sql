-- Adds scaffolds.client_name (Part 36) to the scaffold_inspection share
-- projection so the public read-only report view can show it too — the
-- authenticated PDF path already gets it for free via getScaffold()'s
-- select("*"); this is the one other place a scaffold row is manually
-- re-projected field-by-field, so it needs its own explicit addition.
