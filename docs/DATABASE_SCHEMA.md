...
3. **RLS does not depend on an "active company" JWT claim.** The original design (§8, preserved below as a documented future enhancement) resolves a single active company from a Custom Access Token Auth Hook. That hook must be configured at the Supabase project/dashboard level — out of reach from a migration file, and still unconfigured (see the implementation report). Instead, the implemented helper functions (`is_company_member(target_org_id)`, `has_company_role(target_org_id, role_name)`, `has_any_company_role(target_org_id, role_names)`) take an explicit company id and check membership **per row**, for whichever company that row belongs to. Tenant isolation is fully enforced either way; what the JWT-claim approach adds on top is convenience (not needing to pass an company id around) once that hook exists.
...
`platform_super_admins` (below) is **not implemented** in this milestone — there is no self-service or in-app company creation yet (see [PRODUCT_REQUIREMENTS.md §3](./PRODUCT_REQUIREMENTS.md#3-non-goals-initial-release)), so nothing yet needs to check for platform-super-admin status. Companies are created directly via `supabase/seed.sql` (development) or an equivalent manual/service-role operation (staging/production), matching "manual company onboarding for v1."
...
- Every **tenant-owned** table has `company_id uuid not null references companies(id)`.
...
checked per row against whichever `company_id` that row belongs to — there is no single cached "current company" involved. The originally-proposed alternative — a single active company resolved once from a JWT claim (`company_id = current_org_id()`) — is preserved as a documented future enhancement in [§8.2](#82-original-design-future-enhancement-not-yet-built); it is not what's implemented. Either way, `company_id` is never read off a `profiles` row — `profiles` doesn't carry one (see [ARCHITECTURE.md §3.2](./ARCHITECTURE.md#32-company-membership-model)).

## 2. Enums

| Enum | Values |
|---|---|
| `membership_status` | `invited`, `active`, `suspended`, `removed` — status of an `company_memberships` row. Replaces the earlier `user_status` (which lived on `profiles`; membership status is now per-company, not global). **Implemented.** |
| `company_status` | `trial`, `active`, `suspended`. **Implemented.** |
...
### `companies` — **[global]**, tenant root — **Implemented**
| Column | Type | Notes |
|---|---|---|
| `id` | uuid PK | |
| `name` | text not null | |
...
| `employee_number_prefix` | text not null | **Implemented** (Employee Management Polish milestone). Permanent, uppercase, URL-safe (`^[A-Z0-9]+$`) prefix for this company's generated employee numbers — see [`employees` — Employee number generation](#employees--tenant--implemented). Changing `name` never changes this. |
| `created_at` / `updated_at` | timestamptz | |
| `deleted_at` | timestamptz null | Only a Platform Super Admin action sets this (company offboarding); company data is retained, not purged, on deactivation. |

No `company_id` column (it *is* the tenant root). **Created only by a Platform Super Admin in v1** — see [PRODUCT_REQUIREMENTS.md §3](./PRODUCT_REQUIREMENTS.md#3-non-goals-initial-release). Target RLS design: `INSERT`, and changes to `status`/`deleted_at`, are PSA-only; a `UPDATE` of ordinary settings columns (`name`, future config columns) is additionally allowed for a Company Admin of that company; `SELECT` is allowed for anyone with **any** `company_memberships` row (any status) referencing this company, so an company switcher can show its name even for a not-currently-active membership. See [§8](#8-row-level-security-approach).

**As implemented this milestone** (no `platform_super_admins` table yet — see below): `SELECT` is granted to any authenticated user with an **active** membership (`is_company_member(id)`); there is no `INSERT`/`UPDATE`/`DELETE` policy for the `authenticated` role at all — company creation and settings changes are a service-role/manual operation only (`supabase/seed.sql` for development). The "any status, not just active" nuance above and Company-Admin self-service settings updates are deferred along with `platform_super_admins`.

### `platform_super_admins` — **[global]** — Not implemented this milestone
| Column | Type | Notes |
|---|---|---|
| `user_id` | uuid PK, references `auth.users(id)` | Allow-list of platform operators. Deliberately independent of `company_memberships` — see [ARCHITECTURE.md §3.1](./ARCHITECTURE.md#31-tenant-boundary): Platform Super Admin access must not depend on ordinary tenant membership. |
| `created_at` | timestamptz | |

Deferred: there is no in-app or self-service company creation yet for this table to gate (see [Implementation Status](#implementation-status)); building it is the natural next step once an actual Platform Super Admin onboarding flow is needed.

### `profiles` — **[global]**, identity only — **Implemented**
1:1 with `auth.users`. Holds **identity information only** — no company, no role, no per-company status. Those live in `company_memberships` / `membership_roles` below.
...
| `phone` | text null | |
| `active_company_id` | uuid null, FK `companies(id)` | UX preference only — which company to default into. **Not** a security boundary; every RLS decision re-validates against `company_memberships` regardless of this value. See [ARCHITECTURE.md §3.2](./ARCHITECTURE.md#32-company-membership-model). |
| `user_number` | text not null, unique | **Implemented** (`supabase/migrations/20260726110000_profile_user_number.sql`). Permanent, globally unique, publicly-safe public identifier — format `USR-XXXXXXXX`, an 8-character random Crockford-Base32-style code (`0-9`, `A-Z` minus `I`/`L`/`O`/`U`). Generated automatically by `handle_new_user()` at signup via `generate_user_number()` (bounded retry-on-collision loop; the `unique` constraint is the real guarantee). Immutable once set (`profiles_prevent_user_number_change` trigger — same "RLS allows the row, a trigger blocks the specific column" pattern as `employees.employee_number`). Deliberately **random, not sequential**, unlike `employees.employee_number` — this identifier is platform-wide with no per-tenant RLS boundary narrowing who it identifies, so it must not be trivially enumerable. Entirely independent of any company, membership, or employee record — a person keeps the same `user_number` for life even as their per-company `employees.employee_number` differs in every company they ever work for. Intended as the primary way a future Platform Super Admin area identifies/searches users platform-wide (`/platform/users/USR-...`); no such UI exists yet — this migration is database-only, since `profiles` rows are only ever created by `handle_new_user()`, never by application code directly. |
| `created_at` / `updated_at` | timestamptz | |

No `deleted_at` — a person's identity isn't "deleted" when they leave an company; that's expressed by their `company_memberships.status` becoming `removed`. No `created_by`/`updated_by` — "who invited this person" is tracked on the relevant `company_memberships.created_by` instead, which is the meaningful attribution (an identity can be created by self-signup completing an invite, not by another user acting on the `profiles` row itself).

**Indexes**: `(active_company_id)` — added preemptively in the actual migration rather than waiting for it to show up as a hot path; cheap on a table this shape, and the switcher's default-company lookup is a near-certain future query. `user_number`'s `unique` constraint carries its own backing index.

### `company_memberships` — **[tenant]** — **Implemented**
Connects a `profiles` row to an `companies` row. A person may hold memberships in more than one company; at most one membership row per `(company_id, user_id)` pair.
...
| `created_at` / `updated_at` / `created_by` / `updated_by` | | `created_by` is the admin (or Platform Super Admin, for an company's first Company Admin) who created the invite. |

**Constraints**: `unique (company_id, user_id)`. **Indexes**: `(company_id, status)`, `(user_id)` — the latter specifically to support "list every company I belong to" without an company filter, per [ARCHITECTURE.md §3.2](./ARCHITECTURE.md#32-company-membership-model). Also indexed on `(created_at)` in the actual migration, for future admin/audit views ordered by recency.

### `roles` — **[tenant]** — **Implemented**
The fixed v1 role catalogue, implemented as a reference table rather than the `user_role` enum originally specified here — see [Implementation Status](#implementation-status) for why. All v1 rows are system-defined; not company-configurable (matches [ROLES_AND_PERMISSIONS.md §1](./ROLES_AND_PERMISSIONS.md#1-role-definitions) exactly — the *mechanism* changed, not which roles exist or what they mean).
...
| `is_system` | boolean not null default `true` | Every v1 row is system-defined; the column exists so a future company-custom-role feature (not in scope) has somewhere to record the distinction without a schema change. |
...
|---|---|---|
| `id` | uuid PK | |
| `company_id` | uuid not null, FK `companies(id)` | Denormalized from the parent membership for RLS simplicity/performance, same pattern used by other join tables in this schema (e.g., `lmra_participants`). |
| `membership_id` | uuid not null, FK `company_memberships(id)` | |
...
**Constraints**: `unique (membership_id, role_id)`. **Indexes**: `(membership_id)`, `(company_id)`, `(role_id)` — the latter two support "everyone holding role X in this company" (e.g., resolving HSEQ Manager notification recipients) and general role-catalogue joins.

## 4. Core Tables (continued)

### `employees` — **[tenant]** — **Implemented**
The company employment record for a person working for the company — see [PRODUCT_REQUIREMENTS.md §5.3](./PRODUCT_REQUIREMENTS.md#53-employees). Distinct from `profiles` because a worker may exist on the crew before (or without ever) getting platform login access, and because `profiles` is now a global identity table not scoped to this company. This is the only "who works here" record in the platform — no separate contractor/visitor/external-person table exists or is planned.
...
**Constraints**: unique `(company_id, employee_number)` (the original partial index, `where employee_number is not null`, still exists unchanged — harmless now that the predicate is always true rather than being dropped and recreated, per this milestone's "don't edit an already-applied migration" rule); partial unique `(company_id, profile_id) where profile_id is not null` (an account can back at most one employee record per company); `first_name`/`last_name` non-blank; `work_email` lowercase; `end_date >= start_date`. **Indexes**: `(company_id, account_status)`, `(company_id, employment_status)`, `(company_id, last_name, first_name)`, `(company_id, archived_at)` (added by the correction pass — see [Employee search & pagination](#employee-search--pagination) below), `(profile_id)`.
...
- **`companies.employee_number_prefix`** (`text not null`, added to the already-implemented `companies` table — see that table's section above) — a permanent, uppercase, URL-safe (`^[A-Z0-9]+$`, enforced by a check constraint) prefix, e.g. `VALUTRIS`. Set once; renaming an company's display name never changes it. Backfilled for every pre-existing company by uppercasing and stripping non-`[A-Z0-9]` characters from `slug` (falling back to a deterministic `ORG<8 hex chars of id>` if that strips to nothing) — this already produces `VALUTRIS` for an company whose slug is exactly `valutris`. A prior revision of this migration additionally ran a broad `UPDATE companies SET employee_number_prefix = 'VALUTRIS' WHERE lower(name) LIKE 'valutris%'`; that was removed during the correction pass because a partial name match could silently repoint more than one company onto the same prefix. If the real Valutris company's slug turns out not to be exactly `valutris`, see the implementation report for the exact single-row manual `UPDATE ... WHERE id = '<confirmed-id>'` to run once its id/slug is confirmed — nothing in the migration guesses at it.
- **`company_employee_number_counters`** — **[tenant]**, one row per company (`company_id` PK, `next_number integer not null default 1`, `updated_at`). No RLS policy and no grant to `authenticated`/`anon`/`PUBLIC` at all (explicit `revoke all ... from public, anon, authenticated` in the migration, on top of RLS being enabled+forced with zero policies) — this table has zero direct API surface; it is only ever touched from inside `allocate_employee_number()`. Chosen over deriving the next number from `count(*)`/`max(...)` over `employees` **at allocation time** because neither is safe under concurrent inserts (two concurrent counts can read the same value before either insert commits); a dedicated counter row updated via a single atomic `UPDATE ... RETURNING` is safe because Postgres row-level locking serializes concurrent updates to that one row. `max(...)` **is** used once, at migration time only, to correctly *initialize* this counter from pre-existing data — see the next bullet.
- **Counter initialization (the correction pass's fix)**: naively starting every company's counter at 1 would re-issue a number that already exists on a legacy/manually-entered row (e.g. an existing `VALUTRIS-00001` would be handed out again to the next new employee). Before any backfill runs, the migration computes, per company, the highest numeric suffix among its *existing* `employee_number` values that actually match `<that company's prefix>-<digits>`, and initializes `next_number` to one past that (or `1` if none match). Values that don't match that shape — malformed data, numbers issued under a different prefix, free text entered before this milestone existed — are excluded from that computation and left completely untouched; they're never candidates for "the highest number in use," and (since they're non-null) are never touched by the backfill either. A second, idempotent synchronization pass re-checks the same computation after backfill completes, as a safety net.
- **`allocate_employee_number(target_org_id uuid) returns text`** — `SECURITY DEFINER`, the unauthenticated-safe core allocator (reads the company's prefix, atomically increments its counter, formats `PREFIX-00001`). `revoke all ... from public, anon, authenticated` — not callable by any application role at all; the only callers are `next_employee_number()` below and the numbering migration's own backfill/initialization steps (which run as the migration owner, with no `auth.uid()` session to authorize against in the first place).
- **`next_employee_number(target_org_id uuid) returns text`** — `SECURITY DEFINER`; the real, RPC-exposed function, granted to `authenticated` only. Requires the caller to hold `company_admin` or `operations_manager` **via an active membership** in `target_org_id` (via `has_any_company_role`, which itself requires `company_memberships.status = 'active'` as part of its own check — verified by re-reading that function's definition; an `invited`/`suspended`/`removed` membership, or a role attached to one, can never authorize an allocation) — mirrors `employees_insert_managers` exactly, so number generation and the insert it feeds can never disagree about who's allowed to create an employee. Called once per `createEmployee` Server Function invocation (`modules/employees/actions.ts`); the returned number is then written directly into the `INSERT`.
...
**Constraints**: `end_date is null or end_date >= start_date`; `end_date`/`end_reason` set together or not at all. **Indexes**: unique partial `(employee_id) where end_date is null` (the one-open-period rule above), `(employee_id, start_date desc)`, `(company_id)`.

**Update is deliberately near-immutable** — a `BEFORE UPDATE` trigger (`validate_employment_period_update`/`employee_employment_periods_validate_update`) allows exactly two shapes and rejects everything else, for every role: (a) correcting the `start_date` of a still-**open** period, or (b) closing an open period (`end_date`/`end_reason` set, `start_date` unchanged). Editing an already-**closed** period, reopening one, or changing which employee/company a row belongs to are all rejected unconditionally. A second `BEFORE INSERT OR UPDATE` trigger (`validate_employment_period_no_overlap`) additionally rejects a new or corrected `start_date` that falls on or before the employee's own most recent prior period's `end_date`, so history can never overlap itself. **No `DELETE` grant and no `DELETE` policy at all** — history is never removed, matching the `employees`/`audit_events` pattern.
...
|---|---|---|
| `id` | uuid PK | |
| `company_id` | uuid not null, FK `companies(id) on delete cascade` | Also carries a `unique (id, company_id)` constraint (redundant with `id` alone, added specifically to let `teams`/`project_assignments` composite-FK against this table — see the Review/Hardening Pass note below). |
| `name` | text not null | Checked non-blank. |
| `client_name` | text null | |
| `code` | text null | **Unique per company when present** — `unique (company_id, code) where code is not null`. Unlike `employees.employee_number`, never auto-generated; a manager types it in (or leaves it blank). |
| `description` | text null | |
...
**Constraints**: `name` non-blank; `end_date >= start_date` when both set; `unique (id, company_id)`. **Indexes**: `(company_id, status)`, unique partial `(company_id, code) where code is not null`.
...
**Review/Hardening Pass note (company-boundary integrity)**: every child table below (`project_assignments`, `teams`, `team_assignments`) references this table via a **composite foreign key** on `(id, company_id)` (or, for `team_assignments`, `(team_id, project_id, company_id)` against `teams`), not a plain `references projects(id)`. This makes "a child row's `company_id` doesn't match its parent's" a constraint violation at the database level, not just an application-layer bug — the first version of this migration relied on application code alone to keep these in sync.
...
|---|---|---|
| `id` | uuid PK | |
| `company_id` | uuid not null, FK `companies(id) on delete cascade` | Denormalized from `project_id`, same pattern as `membership_roles.company_id`. |
| `project_id` | uuid not null | Composite FK `(project_id, company_id) references projects (id, company_id)` — see the company-boundary note under `projects` above. |
| `employee_id` | uuid not null | Composite FK `(employee_id, company_id) references employees (id, company_id)` — same reasoning; guarantees the assignee actually belongs to this company. |
...
| `id` | uuid PK | |
| `company_id` | uuid not null, FK `companies(id) on delete cascade` | |
| `project_id` | uuid not null | Composite FK `(project_id, company_id) references projects (id, company_id)`. Also carries `unique (id, project_id, company_id)` so `team_assignments` can composite-FK against *this* table and get "the assignment's `project_id` matches its own team's `project_id`" enforced in one constraint. |
| `name` | text not null | Checked non-blank. |
...
**Constraints**: `name` non-blank; `unique (id, project_id, company_id)`. **Indexes**: `(project_id, display_order)`, `(company_id)`. **Deletion behavior**: no `DELETE` policy or grant — `status = 'archived'` only.
...
| `id` | uuid PK | |
| `company_id` | uuid not null, FK `companies(id) on delete cascade` | |
| `project_id` | uuid not null | Denormalized from `team_id` — what makes the one-active-team-per-*project* index a single-column partial unique index below, and keeps the visibility RLS on `projects`/`employees` from needing an extra join through `teams`. Composite FK `(team_id, project_id, company_id) references teams (id, project_id, company_id)` — one constraint that guarantees `team_id` is real, AND that this row's `project_id`/`company_id` exactly match that team's own (a `team_assignments` row can never claim a `project_id` other than its team's actual project). |
| `team_id` | uuid not null | See composite FK above. |
| `employee_id` | uuid not null | Composite FK `(employee_id, company_id) references employees (id, company_id)`. |
...
**Constraints**: unique partial `(project_id, employee_id) where end_at is null`. **Indexes**: `(team_id) where end_at is null`, `(employee_id)`, `(project_id, employee_id)`, `(company_id)`.
...
**Indexes**: `(company_id, project_id)`, `(parent_location_id)`.
...
**Constraints**: `unique (employee_id, scheduled_date) where status <> 'cancelled' and deleted_at is null` — an employee has at most one active assignment per day. **Indexes**: `(company_id, project_id, scheduled_date)`, `(employee_id, scheduled_date)`.
...
**Constraints**: `unique (employee_id, project_id, work_date) where deleted_at is null`. **Indexes**: `(company_id, project_id, work_date)`, `(employee_id, work_date)`, `(status)`.
...
**Indexes**: `(company_id, status)`, `(timesheet_id)`.
...
**Constraints**: `unique (employee_document_id, milestone) where milestone <> 'post_expiry_unresolved'` — each one-time milestone fires exactly once per document. `post_expiry_unresolved` is exempt from that uniqueness (it recurs while the document remains expired and unrenewed); the job instead checks the most recent `post_expiry_unresolved` row for that document and only fires again after a fixed interval (e.g., 7 days) has passed. **Indexes**: `(employee_document_id, milestone)`, `(company_id, sent_at)`.
...
> **Deferred**: the fixed milestone list and recipient set above are v1 defaults, not yet company-configurable. A future `notification_rules` table (keyed by `company_id` + `document_type`) is the natural extension point if per-company configuration is needed — not built now, per [ARCHITECTURE.md §13](./ARCHITECTURE.md#13-explicitly-deferred-avoid-premature-complexity).
...
Configurable classification used by Incident Reports (and optionally Safety Observations) — see [PRODUCT_REQUIREMENTS.md §6.6](./PRODUCT_REQUIREMENTS.md#66-incident-reports). A hybrid table: a fixed set of **system** rows (global, seeded by migration, `company_id is null`) that every company sees and cannot rename/remove, plus optional **custom** rows an company adds for itself.

| Column | Type | Notes |
|---|---|---|
| `id` | uuid PK | |
| `company_id` | uuid null, FK `companies(id)` | Null for system categories. |
...
**Constraints**: `check ((is_system and company_id is null and system_key is not null) or (not is_system and company_id is not null and system_key is null))`. `unique (system_key) where is_system`. `unique (company_id, name) where not is_system`.
...
| `id` | uuid PK | |
| `company_id` | uuid not null, FK | |
| `project_id` | uuid not null, FK `projects(id)` (composite, `(id, company_id)`) | Immutable after creation. |
...
|---|---|---|
| `id` | uuid PK | |
| `company_id` | uuid not null, FK | Denormalized, same rationale as `lmra_participants`. |
...
|---|---|---|
| `id` | uuid PK | |
| `company_id` | uuid not null, FK | Denormalized for RLS simplicity/performance (avoids a join through `lmra_assessments` in every policy). |
| `lmra_assessment_id` | uuid not null, FK `lmra_assessments(id)` | |
...
Same shape as `lmra_participants`: `id`, `company_id`, `toolbox_talk_id` FK, `employee_id` FK, `signature_id` FK null. `unique (toolbox_talk_id, employee_id)`.
...
**Indexes**: `(company_id, next_inspection_due)` for due-inspection dashboards.
...
| `location_id` | uuid null, FK `project_locations(id)` | |
| `category_id` | uuid not null, FK `event_categories(id)` | Replaces the earlier fixed `incident_type` enum — see [`event_categories`](#event_categories--tenant--global-system-rows). The FK guarantees the category *exists*; the [cross-reference validation rule](./ARCHITECTURE.md#34-cross-reference-validation-rule) still applies to confirm it's either a system category or belongs to this record's `company_id` (a real FK alone doesn't prove the latter). |
...
**Indexes**: `(company_id, status)`, `(company_id, severity, occurred_at)`, `(category_id)`.
...
**Indexes**: `(company_id, status, due_date)` — the primary dashboard query. `(assigned_to, status)`. `(source_type, source_id)`.
...
| `file_path` | text not null | Supabase Storage object key, namespaced `company_id/entity_type/entity_id/...`. |
...
- **Con**: referential integrity for these columns is **not** enforced by the database — it is enforced entirely by the application, per the [Cross-Reference Validation Rule](./ARCHITECTURE.md#34-cross-reference-validation-rule). Concretely, every Server Function that writes one of these columns must, before the write:
  ...
  3. Confirm the referenced row's `company_id` matches this record's `company_id`.
...
|---|---|---|
| `id` | uuid PK | |
| `company_id` | uuid null, FK, `on delete set null` | Null only for platform-level actions (e.g., Platform Super Admin suspending an company). `SET NULL` rather than `CASCADE` — a hard delete elsewhere should never silently delete audit evidence. |
...
**Deletion behavior**: none — no `UPDATE`/`DELETE` RLS policy for any role, including Company Admin, per [ARCHITECTURE.md §8](./ARCHITECTURE.md#8-audit-logging) — **and, as implemented, backed by a second, independent layer**: a hard `BEFORE UPDATE`/`BEFORE DELETE` trigger unconditionally rejects both operations regardless of role, so even `service_role`/`postgres` (which bypass RLS entirely) cannot alter history through normal DML. **Indexes**: `(company_id, created_at desc)`, `(entity_type, entity_id)`, `(actor_user_id)`, `(created_at)`.

## 7. Deletion Behavior Summary

| Table | Deletion behavior | Rationale |
|---|---|---|
| `companies` | Soft delete, Platform Super Admin only | Tenant offboarding retains data for compliance/export. |
| `profiles` | **No deletion** (identity persists) | A person's identity isn't tied to any one company; membership end is expressed via `company_memberships.status = 'removed'`, not a `profiles` deletion. |
| `company_memberships` | **Status-based** (`status = 'removed'`), no separate `deleted_at` | The status enum already models "no longer a member" as a state — see [ARCHITECTURE.md §9](./ARCHITECTURE.md#9-soft-deletion). |
...
- **`is_company_member(target_org_id uuid) returns boolean`** → true if the caller has an **active** `company_memberships` row for `target_org_id`.
- **`has_company_role(target_org_id uuid, role_name text) returns boolean`** → true if the caller has an active membership in `target_org_id` **and** holds `role_name` there (via `membership_roles` → `roles`).
- **`has_any_company_role(target_org_id uuid, role_names text[]) returns boolean`** → same as `has_company_role`, but true if the caller holds **any** of `role_names` there. Added for the Employee Management Foundation milestone to avoid an unreadable 6–8-way OR chain of `has_company_role` calls in the `employees` policies below — a thin, justified extension of the same pattern, not a second role-checking mechanism.

All three are `SECURITY DEFINER` for the same reason: they are called *from within* RLS policies on `company_memberships`/`membership_roles`, and a `SECURITY INVOKER` version's own internal query against those same tables would re-trigger the calling policy — "infinite recursion detected in policy." Running as the function owner (which has `BYPASSRLS` under standard Supabase project setup) breaks that cycle. None of the three is granted to `PUBLIC`; all are revoked from `PUBLIC` and re-granted to `authenticated` only, and none is meant to be called as a public RPC endpoint — they're internal building blocks for policies.

Unlike the original design below, **neither function resolves a single "current active company" from a JWT claim.** Both take an explicit `target_org_id` and check membership/role **per row**, for whichever company that specific row belongs to. This sidesteps needing the Custom Access Token Auth Hook (§8.2) — which requires Supabase project/dashboard configuration this milestone doesn't have access to — without weakening tenant isolation: every policy below still resolves, for every row, to a real, live database check against `company_memberships`.
...
`company_employee_number_counters` (added by the Employee Management Polish milestone) has RLS enabled and forced like every table here, but **zero policies of any kind and zero grants to `authenticated`** — it has no direct API surface at all; every read/write happens exclusively inside `allocate_employee_number()`, a `SECURITY DEFINER` function that runs as its owner regardless of the calling role's own grants.
...
The rest of this section is preserved as-written from the original proposal — a single "active company" resolved from a JWT claim, refreshed via a Custom Access Token Auth Hook. It remains a reasonable direction if/when that hook is configured (it would mostly add ergonomics — not passing an company id around — on top of what §8.1 already enforces), but nothing below is implemented:
...
- **`current_org_id()`** → reads the `org_id` claim from `auth.jwt()` (set by the Custom Access Token Auth Hook — see [ARCHITECTURE.md §3.2](./ARCHITECTURE.md#32-company-membership-model)) and **re-validates it live**: returns the company id only if `company_memberships` has an `active` row for `(auth.uid(), org_id)`; otherwise returns `null`. This means a stale or forged claim — e.g., a token minted before a membership was suspended — can never grant access, since every tenant policy's `company_id = current_org_id()` check fails cleanly against `null`.
- **`current_role_ids()`** → returns the caller's role names (from `roles.name`, via `membership_roles`) for the **currently active** membership (`company_memberships` row matching `current_org_id()`). Resolved live on every call, not baked into the JWT, so a role change takes effect on the very next request without needing a token refresh — unlike `current_org_id()`, which is a coarser, user-initiated switch.
- **`is_platform_super_admin()`** → checks `platform_super_admins` for `auth.uid()`. Deliberately independent of `current_org_id()`/`company_memberships` — see [ARCHITECTURE.md §3.1](./ARCHITECTURE.md#31-tenant-boundary).

Every **[tenant]** table gets, at minimum:
```sql
create policy tenant_isolation_select on <table>
  for select using (company_id = current_org_id());

create policy tenant_isolation_write on <table>
  for insert with check (company_id = current_org_id());
```
...
- **`profiles`**: `select` allowed for the caller's own row, or any row belonging to a person who shares an **active** membership with the caller in at least one common company (needed so the UI can show "Assigned to: Jane Doe" for a colleague). `update` allowed only for the caller's own row.
- **`company_memberships`**: an additional `select` policy allows a user to read their **own** membership rows across **all** companies and statuses (not just the active one) — required for the company switcher, per [ARCHITECTURE.md §3.2](./ARCHITECTURE.md#32-company-membership-model) — layered on top of the standard tenant-scoped policy that lets company admins (`current_role_ids()` containing `company_admin` or `operations_manager`) see other members' rows within the active company. `insert`/`update` (inviting, changing status/roles) allowed for Company Admin within the active company, **or** for `is_platform_super_admin()` — the latter specifically so a Platform Super Admin can provision an company's first Company Admin without needing a membership of their own.
- **`membership_roles`**: mirrors `company_memberships`.
- **`companies`**: `select` allowed for anyone with any `company_memberships` row (any status) referencing the company, or `is_platform_super_admin()`. `insert`, and `update` of `status`/`deleted_at`, restricted to `is_platform_super_admin()`. `update` of other columns (name, settings) additionally allowed for Company Admin of that company.
...
- **`event_categories`**: `select` allowed for system rows (`company_id is null`) to anyone authenticated, or for custom rows matching `current_org_id()`. `insert`/`update` of custom rows restricted to `current_role_ids()` containing `company_admin` or `hseq_manager`, and only with `company_id = current_org_id()`; no policy ever grants write access to system rows.
...
Beyond the per-table indexes noted above, every **[tenant]** table gets a leading index on `company_id` (or `(company_id, <most common filter>)` where noted) since it is the predicate on nearly every query via RLS. Foreign key columns are indexed by default per standard Postgres practice for join performance (`employee_id`, `project_id`, `location_id`, `membership_id`, etc., on the tables above).
