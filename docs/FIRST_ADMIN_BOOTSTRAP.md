...
This document explains the one legitimate way a brand new organization gets
...
Every write path that grants an organization role
...
`operations_manager` in that organization. That's correct for ongoing role
management — but it means a **brand new** organization, with zero role
...
chicken-and-egg loop, exactly once, per organization.
...
```sql
bootstrap_first_owner(target_organization_id uuid, target_user_id uuid, notes text default null)
  returns organization_memberships
```

What it does, in order:

1. Verifies both the organization and the target user (`profiles` row) exist.
2. **Refuses to run at all if the organization already has an active
   ...
   organization has its first admin, every subsequent role change goes
...
3. Creates (or reactivates) an `organization_memberships` row for the
   target user, `status = 'active'`.
4. Assigns the `company_admin` role via `membership_roles`.
5. Writes a row to `bootstrap_audit_log` — organization, user, the role
...
- **Self-limiting.** Once an organization has an active `company_admin`,
  every future call for that same organization raises an exception. There
  is no version of this function that can be used to add a second admin,
  demote someone, or touch a different organization's existing admin — that
  is deliberate.
- **Always audited.** Every successful call leaves a permanent
  `bootstrap_audit_log` row, readable by that organization's
...
If the organization already has a `company_admin`, this raises a Postgres
exception (`P0001`) instead of silently doing nothing — treat that as "this
organization is already bootstrapped, use the normal Roles tab instead,"
not as a bug to work around.

## What this is *not* for

- Adding a second admin to an already-bootstrapped organization — use the
  existing employee Roles tab (`assignEmployeeRole()`), which any current
  `company_admin` can already do for anyone in their org.
...
- Anything a `platform_super_admin` role/table would eventually own once
  ...
  function is a narrower, org-scoped stopgap for that specific gap, not a
